...
The core engineering interest: the four backends fall into **three** distinct
shapes, so each needs a different wrapping strategy. (gmt-mcp reuses the same
in-process-import pattern as obspy-mcp — PyGMT is, like ObsPy, a Python
library — so it adds a fourth server without adding a fourth pattern.)
...
核心的技术趣味在于：四个后端落入**三种**截然不同的形态，所以各自需要不同的封
装策略。（gmt-mcp 复用了和 obspy-mcp 相同的进程内 import 模式——PyGMT 和
ObsPy 一样是 Python 库——所以它增加了第四个 server，但没增加第四种模式。）
